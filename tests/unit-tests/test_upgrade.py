...
def mock_resource_download():
    def download(source, dest, params):
        resource_name = os.path.basename(dest)
        resource_path = os.path.join(
                utils.BlueprintResourceFactory.RESOURCES_PATH,
                TEST_SERVICE_NAME, resource_name)
        utils.mkdir(os.path.dirname(resource_path))
        utils.write_file('port: 8080', resource_path)
    return download
...
@patch('utils.ctx.download_resource_and_render', mock_resource_download())
@patch('utils.ctx', mock_install_ctx())
def _create_resource_file(resource_dest):
    resource_factory = utils.BlueprintResourceFactory()
    ctx_props, props_file_path = create_install_props_file(TEST_SERVICE_NAME)
    resource_factory.create('', resource_dest,
                            ctx_props, user_resource=False)
...
class TestUpgrade(unittest.TestCase):
    ...
    @patch('utils.ctx', mock_upgrade_ctx())
    @patch('utils.CtxPropertyFactory.ROLLBACK_PROPERTIES_PATH',
           tempfile.mkdtemp())
    @patch('utils.is_upgrade', lambda: True)
    def test_archive_properties(self):
        ...
        factory = utils.CtxPropertyFactory()
        factory.PROPERTIES_PATH = os.path.join(os.path.dirname(install_path),
                                               '../')
        factory.archive_properties(TEST_SERVICE_NAME)

        archived_properties_path = os.path.join(
                utils.CtxPropertyFactory.ROLLBACK_PROPERTIES_PATH,
                TEST_SERVICE_NAME,
    ...
    @patch('utils.BlueprintResourceFactory.RESOURCES_PATH', tempfile.mkdtemp())
    @patch('utils.is_upgrade', lambda: False)
    def test_resource_file_create_on_install(self):
        ...
        resource_local_path = os.path.join(
                utils.BlueprintResourceFactory.RESOURCES_PATH,
                TEST_SERVICE_NAME, TEST_RESOURCE_NAME)
    ...
    def test_resource_file_create_on_update(self):
        ...
        resource_local_path = os.path.join(
                utils.BlueprintResourceFactory.RESOURCES_PATH,
                TEST_SERVICE_NAME, upgrade_file_name)
        self.assertTrue(os.path.isfile(resource_local_path))

    @patch('utils.is_upgrade', lambda: True)
    @patch('utils.ctx', mock_upgrade_ctx())
    @patch('utils.BlueprintResourceFactory.RESOURCES_PATH', tempfile.mkdtemp())
    @patch('utils.BlueprintResourceFactory.RESOURCES_ROLLBACK_PATH',
           tempfile.mkdtemp())
    def test_archive_resources(self):
        ...
        utils.BlueprintResourceFactory().archive_resources(TEST_SERVICE_NAME)
        archived_resource_file = os.path.join(
                utils.BlueprintResourceFactory.RESOURCES_ROLLBACK_PATH,
        ...
        curr_resource_file = os.path.join(
                utils.BlueprintResourceFactory.RESOURCES_PATH,
                TEST_SERVICE_NAME, 'upgrade.conf')
