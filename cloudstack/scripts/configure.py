...
from cloudstack_plugin.floatingip import IP_ADDRESS_PROPERTY
...
def _copy_cloudstack_configuration_to_manager(cloudstack_config):
    ...
    Config.update_config(merged_config, cloudstack_config)

    tmp = tempfile.mktemp()
    with open(tmp, 'w') as f:
        json.dump(merged_config, f)
    # with settings(host_string=manager_public_ip,connection_attempts=5,
    #               timeout=5, keepalive=1):
